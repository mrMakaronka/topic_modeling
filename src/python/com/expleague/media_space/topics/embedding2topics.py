    ...
    def convert(self, embeddings: np.ndarray) -> Optional[np.ndarray]:
        ...
        for embedding in embeddings:
            ...
            if len(embeddings) > 1:
                ...
                if len(ind) == 0:
                    continue
                dict_ind_dist = dict(zip(ind, dist))
                vec_dist = np.array([dict_ind_dist.get(i, 0) for i in range(result_vec.shape[0])], dtype=np.float32)
                result_vec += vec_dist / len(ind)
            else:
                # noinspection PyArgumentList
        ...
        if np.count_nonzero(result_vec) == 0:
            return None
        result_vec = result_vec / np.linalg.norm(result_vec, 2)
        return result_vec
